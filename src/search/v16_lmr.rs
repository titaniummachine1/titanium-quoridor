...
/// Wall move: ACE baseline, unless attention ≤ 10% of max legal impact (dead tail).
pub fn plan_v16_wall_lmr(
    ...
    if attention_ratio <= CAT_ATTENTION_TAIL_CUTOFF {
        return V16LmrPlan {
            ace_base_reduction: ace_base,
            hard_override: V16HardOverride::DeadTail,
            final_reduction: max_safe,
            child_depth_used: 1,
        };
    }
...
mod tests {
    ...
    fn dead_tail_forces_depth_one() {
        let p = plan_v16_wall_lmr(8, 10, 9, 0.05, 1);
    ...
    #[test]
    fn backward_wall_forces_depth_one() {
        let p = plan_v16_wall_lmr(8, 10, 9, 1.0, 0);
    ...
    #[test]
    fn non_progress_pawn_forces_depth_one() {
